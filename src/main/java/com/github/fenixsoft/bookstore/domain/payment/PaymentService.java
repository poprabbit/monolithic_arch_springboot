...
public class PaymentService {
     ...
     * delete~由于支付单的存储中应该保存未持久化的购物明细（在Settlement中），所以这步就不做处理了，等2分钟后在触发器中释放~
     */
     ...
     * 1. 如果要考虑支付订单的~取消~自动冲销场景，无论支付状态如何，这个TimerTask到时间之后都应当被执行。不应尝试使用TimerTask::cancel来取消任务。
     * 因为只有带有上下文状态的节点才能完成~取消~自动冲销操作，如果要在集群中这样做，就必须使用支持集群的定时任务（如Quartz）以保证多节点下能够正常取消任务。
